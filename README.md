...
`retirement_strategy.py` 提供「緩衝池退休法」分頁（在「AI 深度解讀」之後），說明「退休5」「退休6」「退休7」預設配置的提領規則並逐項解釋每個數字。成長池比例讀自 `allocation.py` 的 `PRESETS['退休5']`，預設改動時頁面自動跟著改。**撥出率由滑桿決定（1%–8%，預設 4%）**，領出率固定 25%。**回測用哪個配置由使用者選（退休5／6／7，預設退休6），頁面自行取得那兩檔持股的行情跑一段逐月回測**（線為總資產、長條為每月生活費，**每年 1 月的撥款月用另一個顏色標出**），所以這個分頁現在會打網路 —— 但只在它被開啟時，而且與側邊欄的選股無關。共同期間約 6.8 年，受 00865B 上市日限制 —— 三個配置共用同一檔緩衝池，所以退休6（VT）與退休7（0050）落在同一段期間、可以直接對照；退休5 的 009826 只有幾週，選它會顯示為什麼不能回測並指出實際重疊期間。**退休7 在這段樣本上的數字明顯較好，頁面附上集中度與樣本期間的警語**（見 `BACKTEST_NOTES`）。圖表下方另有一張**並排比較表**，把跑得動的配置放在同一段期間對照 —— 期間取的是它們的交集（`shared_window()`），**歷史不足一整年的配置會先被剔除再取交集**，否則退休5 的幾週會把交集縮到它自己的長度、害所有配置都跑不動。圖上標出成長池跌超過 15% 的期間，另附一張表列出跌超過 8% 的每一次與同期緩衝池的漲跌；**事件名稱取自 `EPISODE_NAMES`，是人工對照而非從資料推出的**，認不出來的留白。頁面也會算出緩衝池最大跌幅裡有多少是匯率（實測 2025 年那 12.1% 幾乎全是台幣升值）。長篇文字解釋收在展開區。
