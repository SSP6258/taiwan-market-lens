...
`retirement_strategy.py` 提供「緩衝池退休法」分頁（在「AI 深度解讀」之後），說明「退休5」「退休6」預設配置的提領規則並逐項解釋每個數字。成長池比例讀自 `allocation.py` 的 `PRESETS['退休5']`，預設改動時頁面自動跟著改。**撥出率由滑桿決定（1%–8%，預設 4%）**，領出率固定 25%。**頁面會自行取得「退休6」兩檔持股的行情跑一段逐月回測**（線為總資產、長條為每月生活費，**每年 1 月的撥款月用另一個顏色標出**），所以這個分頁現在會打網路 —— 但只在它被開啟時，而且與側邊欄的選股無關。共同期間約 6.8 年，受 00865B 上市日限制。圖上標出成長池跌超過 15% 的期間，另附一張表列出跌超過 8% 的每一次與同期緩衝池的漲跌；**事件名稱取自 `EPISODE_NAMES`，是人工對照而非從資料推出的**，認不出來的留白。頁面也會算出緩衝池最大跌幅裡有多少是匯率（實測 2025 年那 12.1% 幾乎全是台幣升值）。長篇文字解釋收在展開區。
