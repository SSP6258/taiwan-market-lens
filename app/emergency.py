...
PLAYBOOK = [
    ...
    {'名稱': '賣成長池換正2', '關鍵': '借不到才考慮',
     ...
     '代價': '**把這條規則的「免再平衡」性質弄壞**，而且是在市場壓力下買進槓桿。'},
    {'名稱': '直接賣成長池', '關鍵': '永遠可用',
...
def render_emergency():
    ...
    with st.expander('正2 那個做法，兩個代價講清楚'):
        st.markdown(
            f'一、**它會腐蝕這條規則最值錢的性質。** 實測（{MEASURED_WINDOW}）把成長池裡 10% '
            '換成 00685L，六年半後它佔成長池由 11.1% 漲到 **33.0%**。'
            '規則從不平衡成長池內部，所以那個比例只會一路漂 —— '
            '而「免再平衡」正是這條規則宣稱的三個結構性優勢之一。\n\n'
            '二、**它維持的是曝險，不是風險。** 急用吃掉一部分淨值之後維持同樣的絕對曝險，'
