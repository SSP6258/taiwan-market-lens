...
# Written out because the page counts the playbook in two places and "5 個錦囊" reads
# badly beside "四個". Derived rather than typed: the count drifted the first time an
# entry was added, and the heading said four while the table showed five.
...
def render_emergency():
    ...
    st.markdown(f'### {_numeral(len(PLAYBOOK))}個錦囊，照這個順序試')
    st.markdown(
        '| | 做法 | | 為什麼排這裡 | 代價 |\n|---|---|---|---|---|\n' +
    ...
    st.error(
        ...
        f'**退休那天起可能就辦不下來了** —— 其他{_numeral(len(PLAYBOOK) - 1)}個退休後都還能用，'
        '只有這個是資格性的，'
        '而資格建立在你即將失去的那樣東西上。\n\n'
    ...
    cards[1].metric('從緩衝池拿 100 萬', f'隔年生活費少 {wan(cost["buffer"])}',
                    ...
                    delta_color='off', delta_arrow='off', border=True)
    st.info(
        '**要質押就押成長池，不要押緩衝池。** 緩衝池每個月都要動 —— 生活費是從那裡'
        '一個月領一次出來的。設定成擔保品之後那個動作會不會卡住，要先問清楚；'
        '成長池一年只在 1 月動一次，影響小得多。')
    st.warning(
    ...
    with st.expander('借錢之前先訂門檻：LTV 與利率'):
        ...
        st.info(
            '**實際報價（' + OBSERVED_SOURCE + '）：**　'
            + '　'.join(f'{name} **{rate:.1%}**' for name, rate in OBSERVED_RATES)
            + '。\n\n**利率是逐檔的，不是一個數字。** 槓桿型要貴上一半 —— '
            '所以如果走了正2 那條路，之後再想質押它，成本比質押原本的持股高。'
            '這是單一券商、單一時點的報價，別家與日後都可能不同。')
    ...
    with st.expander('這一頁的數字從哪裡來，以及不要拿它當什麼用'):
        st.markdown(
            ...
            '理財型房貸的利率與央行信用管制的影響、'
            '（質押利率已有實際報價，見上方展開區。）'
            '以及信用貸款在沒有薪資所得時**能不能用股票庫存當財力證明**、'
            '無擔保總額上限怎麼認定、有沒有循環動用型。'
            '另外**質押設定之後的持股還能不能自由處分**要看契約 —— '
            '那會影響每年 1 月從成長池撥款那個動作。這些都是你問得到、這一頁問不到的。\n\n'
